...
if not path.exists(load_name) then
    ...
    model:add(nn.ReLU())
    model:add(nn.Linear(2000, d))
    --model:add(nn.ReLU())
    model:add(nn.Linear(d, 2000))
    model:add(nn.ReLU())
...
train = function(model, inference_net, trainset, n_points, n_epoches, sample_times, K, delta, lr, lr_decay) 
    -- sgd parameters
    sgd_params.learningRate = 0
    -- sgd_params.weightDecay = math.pow(10, -tag)
    -- sgd_params.momentum = 0.9
    sgd_params.learningRateDecay = 0

    print("sample_times, K, delta, lr, lr_decay, sgd_params.weightDecay: ", sample_times, K, delta, lr, lr_decay, sgd_params.weightDecay)    
    ...
    local feval = function(x_new)
        ...
        for ite = 1, K do
            ...
            dz = dz:cuda() * delta / K / n_points
            loss = loss + (l1_loss + l2_loss) * delta / K / n_points
            print(string.format("   [%s], loss1: %f, loss2: %f", os.date("%c", os.time()), l1_loss * delta / K / n_points, l2_loss * delta / K / n_points))
    ...
    for epoch = 1, n_epoches do
        ...
        if sgd_params.learningRate < 1e-5 then
            sgd_params.learningRate = 1e-5
        end
        _, fs = optim.sgd(feval, x, sgd_params, state)

        if epoch % 50 == 0 then
            -- visualize(trainset.label, model.modules[z_index].output:double(), epoch)
            torch.save(model_name .. string.format("_%d_%d_3", epoch, sample_times), model)
            torch.save(string.format("./results/%d_%d", epoch, sample_times), tables)
        end
...
print("alpha, lambda: ", alpha, lambda)
...
--     gibbs_sample(z, n_points)
-- end
-- torch.save(string.format('params_%d.2.t7', tag), {belong, connect, tables, connected, n_tables})

params = torch.load(string.format('params_%d.2.t7', tag))
belong = params[1] 
connect = params[2] 
tables = params[3]  
connected = params[4]
...
train(model, inference_net, trainset, n_points, n_epoches, 14, 3, 0.3, 0.01, 50)
